...
const NAV: { group: string; items: NavItem[] }[] = [
  ...
  { group: "Grow", items: [
    ...
    { icon: "bi-shield-check", label: "Insurance & Protection", active: true },
      ...
      <aside className={`pm-sidebar ${open ? "open" : ""}`}>
        <div className="pm-brand">
          ...
          <div>
            ...
            <div className="pm-brand-sub">Page 11 · Insurance &amp; Protection</div>
          </div>
        </div>
        ...
        <div className="pm-sidebar-foot">
          <div className="pm-upgrade">
            <div className="d-flex align-items-center gap-2 mb-1">
              ...
              <span className="fw-bold">Coverage radar</span>
            </div>
            KES 24.5M protected · 6 policies · 1 claim in review.
          </div>
      ...
      <div className="ms-auto d-flex align-items-center gap-2">
        <div className="pm-search-box d-none d-lg-block">
          ...
          <input
            ...
            style={{ width: 240 }}
            placeholder="Search policies, claims, beneficiaries…"
            value={searchQuery}
            ...
            <>
              ...
              <div className="pm-dd-menu" style={{ width: 330, right: 0 }}>
                ...
                <button type="button" className="pm-dd-item justify-content-center" onClick={() => { setBellOpen(false); openModal("help"); }}>
                  <i className="bi bi-sliders" /> Policy &amp; renewal settings
                </button>
              </div>
            </>
          )}
                ...
                {BUSINESSES.map((b) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setBusiness(b.name);
                      toast(`Policies and claims now scoped to ${b.name}.`, "info", "Business switched");
                    }}
                  >
                    <span>{b.emoji}</span>
...
export function QuickBar() {
  const { openModal, policies } = useStore();
  const renewing = policies.filter((p) => p.status === "Expiring soon").length;
  return (
    <nav className="pm-quickbar" aria-label="Quick actions">
      <button type="button" className="primary" onClick={() => openModal("claim")}>
        <i className="bi bi-shield-exclamation" /> File a Claim
      </button>
      <button type="button" onClick={() => openModal("policy")}>
        <i className="bi bi-shield-plus" /> Get a Quote
      </button>
      <button type="button" onClick={() => openModal("renewals")}>
        <i className="bi bi-calendar-event" /> {renewing} Renewals Due
      </button>
      <button type="button" onClick={() => openModal("beneficiaries")}>
