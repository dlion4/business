...
const NAV: { group: string; items: NavItem[] }[] = [
  ...
  { group: "Run", items: [
    { icon: "bi-buildings", label: "Multi-Business Portfolio", active: true },
      ...
      <aside className={`pm-sidebar ${open ? "open" : ""}`}>
        <div className="pm-brand">
          ...
          <div>
            ...
            <div className="pm-brand-sub">Page 14 · Multi-Business Portfolio</div>
          </div>
        </div>
        ...
        <div className="pm-sidebar-foot">
          <div className="pm-upgrade">
            <div className="d-flex align-items-center gap-2 mb-1">
              <i className="bi bi-buildings" style={{ color: "#ffd66b" }} />
              <span className="fw-bold">One login, every business</span>
            </div>
            5 entities · 2 rental properties · free inter-company transfers · consolidated P&amp;L.
          </div>
  ...
  return (
    <header className="pm-topbar">
      ...
      <div className="pm-crumb d-none d-md-block">
        Run / <b>Multi-Business Portfolio</b>
      </div>
      <div className="ms-auto d-flex align-items-center gap-2">
        <div className="pm-search-box d-none d-lg-block">
          ...
          <input
            ...
            style={{ width: 240 }}
            placeholder="Search entities, tenants, transfers…"
            value={searchQuery}
                ...
                {BUSINESSES.map((b) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setBusiness(b.name);
                      toast(`Portfolio view now scoped to ${b.name}.`, "info", "Business switched");
                    }}
                  >
                    <span>{b.emoji}</span>
...
export function QuickBar() {
  ...
  return (
    <nav className="pm-quickbar" aria-label="Quick actions">
      <button type="button" className="primary" onClick={() => openModal("entityWizard")}>
        <i className="bi bi-plus-lg" /> New Entity
      </button>
      <button type="button" onClick={() => openModal("transferWizard")}>
        <i className="bi bi-arrow-left-right" /> Transfer Funds
      </button>
      <button type="button" onClick={() => openModal("tenantWizard")}>
        <i className="bi bi-person-plus" /> Add Tenant
      </button>
      <button type="button" onClick={() => openModal("loanWizard")}>
        <i className="bi bi-cash-stack" /> Record Loan
      </button>
      <button type="button" onClick={() => openModal("maintenanceWizard")}>
        <i className="bi bi-tools" /> Maintenance Job
      </button>
