...
const NAV: { group: string; items: NavItem[] }[] = [
  ...
  { group: "Money Out", items: [
    { icon: "bi-truck", label: "Pay Suppliers", badge: "2 POs" },
  ]},
  ...
  { group: "Your Business", items: [
    ...
    { icon: "bi-boxes", label: "Inventory & Stock", badge: "3 alerts", active: true },
      ...
      <aside className={`pm-sidebar ${open ? "open" : ""}`}>
        <div className="pm-brand">
          ...
          <div>
            ...
            <div className="pm-brand-sub">Page 8 · Inventory &amp; Stock</div>
          </div>
        </div>
            ...
            <div key={g.group}>
              ...
              {g.items.map((it) => (
                <a
                  ...
                  onClick={() => {
                    if (it.active) return;
                    toast(`This build focuses on PAGE 8 — Inventory & Stock. "${it.label}" ships in its own page.`, "info", "PayMo demo");
                  }}
                >
...
export function Topbar({ onMenu }: { onMenu: () => void }) {
  ...
  const notifAction = (action?: string) => {
    if (!action) return;
    if (action === "Reorder") openModal("reorderWizard");
    else if (action === "View PO") openModal("poDrawer", { id: "PO-1041" });
    else if (action === "Resume") openModal("countWizard", { id: "CNT-113" });
    else if (action === "Write off") openModal("writeoffWizard");
    setBellOpen(false);
  };
  ...
  return (
    <header className="pm-topbar">
      ...
      <div className="pm-crumb d-none d-md-block">
        Your Business / <b>Inventory &amp; Stock</b>
      </div>
              ...
              <div className="pm-dd-menu" style={{ width: 340, right: 0 }}>
                ...
                {notifications.map((n) => (
                  <div key={n.id} className="pm-dd-item" style={{ alignItems: "flex-start", opacity: n.unread ? 1 : 0.6 }}>
                    ...
                    {n.action && (
                      <button type="button" className="btn btn-link btn-sm p-0 text-primary" style={{ fontSize: "0.66rem" }} onClick={() => notifAction(n.action)}>
                        {n.action} →
                      </button>
                    )}
                  </div>
                ...
                <hr />
                <button type="button" className="pm-dd-item justify-content-center" onClick={() => { setBellOpen(false); openModal("alerts"); }}>
                  <i className="bi bi-sliders" /> View all inventory alerts
                </button>
              </div>
            </>
          )}
                ...
                {BUSINESSES.map((b) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (b.current) return;
                      setBusiness(b.name);
                      toast(`Context switched to ${b.name}. Stock levels, counts and POs now scoped to this entity.`, "info", "Business switched");
                    }}
                  >
                    <span>{b.emoji}</span>
          ...
          {accOpen && (
            <>
              ...
              <div className="pm-dd-menu" style={{ width: 210, right: 0 }}>
                <button type="button" className="pm-dd-item" onClick={() => { setAccOpen(false); openModal("location"); }}><i className="bi bi-geo-alt" /> Manage locations</button>
                ...
                <button type="button" className="pm-dd-item" onClick={() => { setAccOpen(false); openModal("valuation"); }}><i className="bi bi-calculator" /> Valuation method</button>
...
export function QuickBar() {
  ...
  return (
    <nav className="pm-quickbar" aria-label="Quick actions">
      <button type="button" className="primary" onClick={() => openModal("countWizard")}>
        <i className="bi bi-clipboard-check" /> Stock Count
      </button>
      <button type="button" onClick={() => openModal("receiveWizard")}>
        <i className="bi bi-box-arrow-in-down" /> Receive PO
      </button>
      <button type="button" onClick={() => openModal("transferWizard")}>
        <i className="bi bi-arrow-left-right" /> Transfer
      </button>
      <button type="button" onClick={() => openModal("adjustmentWizard")}>
        <i className="bi bi-sliders" /> Adjust
      </button>
      <button type="button" onClick={() => openModal("reorderWizard")}>
        <i className="bi bi-cart-plus" /> Reorder
      </button>
      <button type="button" onClick={() => openModal("exportLedger")}>
